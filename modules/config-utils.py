...
def check_fields(config):
    ...
    :raises Exception: Dataproc paths are missing from input json.
    :raises Exception: Script parameters are missing from input json.
    """    
    
    # define expected fields
    level1_keys = ['gnomad_paths','dataproc_paths','script_params']
    gnomad_keys = ['exomes','genomes']
    dataproc_keys = ['local_cache']
    script_params = ['testing', 'allele-frequency-cutoff', 'input_vcf', 'output_path']
    ...
    missing_keys = [i for i in gnomad_keys if i not in config['gnomad_paths'].keys()]
    if missing_keys:
        except_str = f"The following keys are missing from the 'gnomad_keys' level of your config: {', '.join(missing_keys)}."
        raise Exception(except_str)
        
        
    missing_keys = [i for i in dataproc_keys if i not in config['dataproc_paths'].keys()]
    if missing_keys:
        except_str = f"The following keys are missing from the 'dataproc_paths' level of your config: {', '.join(missing_keys)}."
        raise Exception(except_str)    
        
        
    missing_keys = [i for i in script_params if i not in config['script_params'].keys()]
    if missing_keys:
        except_str = f"The following keys are missing from the 'script_params' level of your config: {', '.join(missing_keys)}."
        raise Exception(except_str)   
...
def check_types(configvalue, configtype):
    ...
    :param configtype: Expected type of config ('google-cloud-path','float','boolean','local-path')
    :type configtype: str
    ...
    """    
    types = ['google-cloud-path','float','boolean','local-path']
    ...
    if configtype == 'local-path':
        if not isinstance(configvalue, str):
            raise Exception(f"Input config value {configvalue} must be a string!")
